...
export class PlayerService extends BaseService implements OnDestroy {
    ...
    async init() {
        ...
        this.storedDeviceKey = this.parameterService.getStorageKey(LocalStorageKeys.currentDevice);
        await this.start();
        this.log.information("player service init()");
    ...
    private async getActiveDevices(): Promise<AudioDevice[]> {
        ...
        return new Promise<AudioDevice[]>(resolve => {
            ...
            this.log.debug(`getActiveDevices() returned ${list.length} devices`);
            resolve(devices);
        });
    }
}
