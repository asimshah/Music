...
namespace Fastnet.Music.Metatools
{
    ...
    //                else
    //                {
    //                    var artistOrFilmName = remainingParts.First(); // film name if Hindi Films, else artist name
    //                    if (!artistOrFilmName.StartsWith("$"))
    //                    {
    //                        if (!styleInfo.Filter || styleInfo.IncludeArtists.Any(x => x.IsEqualIgnoreAccentsAndCase(artistOrFilmName)))
    //                        {
    //                            switch (remainingParts.Length)
    //                            {
    //                                case int l when l == 1:
    //                                    var t1 = Path.Combine(source.DiskRoot, ss.Path, artistOrFilmName);
    ...
    //                                            MusicStyle = styleInfo.Style,
    //                                            StylePath = ss.Path,
    //                                            //ArtistPath = artistOrFilmName,
    //                                            IsGenerated = source.IsGenerated,
    //                                            IsCollections = string.Compare(artistOrFilmName, "collections", true) == 0
    //                                        };
    //                                        if (styleInfo.Style == MusicStyles.HindiFilms)
    //                                        {
    //                                            pd.OpusPath = artistOrFilmName;
    //                                        }
    //                                        else
    //                                        {
    //                                            pd.ArtistPath = artistOrFilmName;
    //                                        }
    //                                    }
    //                                    break;
    //                                case int l when l > 1:
    //                                    Debug.Assert(styleInfo.Style != MusicStyles.HindiFilms);
    //                                    var t2 = Path.Combine(source.DiskRoot, ss.Path, artistOrFilmName, remainingParts.Skip(1).First());
